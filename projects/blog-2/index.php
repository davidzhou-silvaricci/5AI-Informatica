...
<body>
  <div class="container mx-auto p-8 min-h-screen mb-12">
    ...
    <section id="lista-articoli" class="grid grid-cols-3 gap-6 grid-flow-row auto-rows-max">
      <?php while ($articolo = $lista->fetch_object()) : ?>
        <a href="<?= $listaArticoli->urlView($articolo->id); ?>" class="border border-black-500 cursor-pointer transition duration-500 ease-in-out hover:shadow-lg rounded-lg">
          <div class="p-8">
            <header>
              <h1 class="font-serif text-2xl mb-4 text-green-600 font-bold"><?= $articolo->title ?></h1>
            </header>
            <section class="leading-relaxed text-gray-500">
              <?= Articolo::getTestoCorto($articolo->text) ?>
            </section>
          </div>
        </a>
      <?php endwhile; ?>
      <?php if ($listaArticoli->getLista()->num_rows === 0) : ?>
