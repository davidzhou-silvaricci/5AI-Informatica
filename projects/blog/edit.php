    ...
    <div class="container mx-auto px-96 py-8 box-border min-h-screen mb-12">
      <?php if(isset($articolo)): ?>
        ...
        <form action="<?= $listaArticoli->urlView($articolo->getId()) ?>" method="POST" enctype="multipart/form-data" class="flex flex-col justify-center align-center">
          ...
          <label class="font-medium text-gray-700" for="upload">Copertina</label>
          <div class="flex flex-col mt-2 mb-6">
            <div class="shrink-0 h-96 rounded-xl overflow-hidden">
              <img id="preview" class="w-full h-full object-cover object-center" src="<?= $articolo->getImg() ?>" alt="Copertina" />
            </div>
            <label class="block mt-4">
              <span class="sr-only">Copertina</span>
              <input type="file" id="image" name="image" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-medium file:bg-orange-50 file:text-orange-700 file:transition-colors hover:file:bg-orange-100"/>
            </label>
          </div>
